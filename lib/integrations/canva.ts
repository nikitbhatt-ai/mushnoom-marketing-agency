...
// dev-time tool, not something a Vercel route can call. The access token is the
// client's own, minted via the in-app "Connect Canva" OAuth flow and kept fresh
// by canva-oauth.getValidAccessToken(); callers pass it in here.
...
async function pollJob(path: string, accessToken: string): Promise<any> {
  for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt++) {
    await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS));
    const res = await fetch(`${CANVA_API}${path}`, { headers: authHeaders(accessToken) });
...
async function autofill(
  ...
  const job = (await res.json()).job;
  const done = job.status === "success" ? job : await pollJob(`/autofills/${job.id}`, accessToken);
...
async function exportPng(designId: string, accessToken: string): Promise<string[]> {
  ...
  const job = (await res.json()).job;
  const done = job.status === "success" ? job : await pollJob(`/exports/${job.id}`, accessToken);
 ...
 * PNG. `accessToken` is the client's own Canva token (from canva-oauth). Returns
 * the design id and one (temporary) PNG URL per page — the caller rehosts those
 * somewhere public before publish (§6).
 */
export async function renderTemplate(
  ...
  const designId = await autofill(brandTemplateId, data, accessToken);
  const pageUrls = await exportPng(designId, accessToken);
