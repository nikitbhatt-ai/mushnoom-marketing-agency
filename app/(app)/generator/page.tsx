...
import { CONTENT_ITEMS, SOURCE_FILES } from "@/lib/mock/data";
import type { ContentFormat, ContentItem, Pillar, Platform } from "@/lib/types";
...
export default function GeneratorPage() {
  const [sourceId, setSourceId] = useState(SOURCE_FILES[0].id);
  ...
  const source = useMemo(
    () => SOURCE_FILES.find((s) => s.id === sourceId)!,
    [sourceId]
  );
  ...
  function generate() {
    ...
    setGenerating(true);
    // Phase 1: simulate generation by pulling matching mock drafts. Phase 3
    // replaces this with POST /api/generate (runAgent + the skill).
    setTimeout(() => {
      const pool = CONTENT_ITEMS.filter((c) => c.sourceFileId === sourceId);
      const made = formats.map((fmt, i) => {
        const template =
          pool.find((c) => c.format === fmt) ?? pool[0] ?? CONTENT_ITEMS[0];
        return {
          ...template,
            ...
            <div className="mt-3 space-y-1.5">
              {SOURCE_FILES.map((s) => (
                <button
                  ...
                  style={{ borderWidth: "0.5px", borderColor: sourceId === s.id ? "#e6e6e6" : "transparent" }}
                >
...
function DraftCard({ item }: { item: ContentItem }) {
  ...
  return (
    <div className="card p-5">
      ...
      <div className="mt-3 rounded-md bg-canvas p-3" style={{ borderWidth: "0.5px", borderColor: "#e6e6e6" }}>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <button className="rounded-md px-3 py-1.5 text-xs text-muted hover:text-ink" style={{ borderWidth: "0.5px", borderColor: "#e6e6e6" }}>
          Regenerate
        </button>
        <button className="rounded-md px-3 py-1.5 text-xs text-muted hover:text-ink" style={{ borderWidth: "0.5px", borderColor: "#e6e6e6" }}>
          Edit
